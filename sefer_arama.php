...
<body>
    ...
    include('giris.php');
    include('uye_ol.php');
    $kalkis = (int)$_POST["kalkis"];
    $varis = (int)$_POST["varis"];
    $k_tarih = $_POST["date-time"];

    //$query = "SELECT * FROM seferler WHERE kalkis='".(int)$_POST["kalkis"]."' and varis= '".(int)$_POST["varis"]."'";
    //$sorgu = $db->prepare( "SELECT * FROM seferler WHERE kalkis=".$kalkis);
    /*
    SELECT 
    s.id,
    fiyat,
    ...
    */
    ...
    tipi,
    kapasite,
    (SELECT guzergah_adi FROM guzergah g WHERE g.id=s.guzergah_id)guzergahlar,
    (SELECT adi FROM duraklar WHERE durak_id=s.kalkis)kalkis_adi,
    (SELECT adi FROM duraklar WHERE durak_id=s.varis)varis_adi
    FROM seferler s
    INNER JOIN firmalar f ON s.firma_id = f.id
    INNER JOIN otobusler o ON s.otobus_id = o.id
    WHERE kalkis='" . $kalkis . "'AND varis='" . $varis . "'AND DATE(k_tarih)='" . $k_tarih . "'");
    $sorgu->execute();
    ...
    <h1 class="modal-title text-center" id="exampleModalLongTitle"></h1>
